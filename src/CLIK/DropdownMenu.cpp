	...
	namespace GFx
	{
		namespace Controls
		{
			...
			std::string_view DropdownMenu::ItemToLabel(Object& a_item)
			{
				...
				RE::GFxValue str;
				[[maybe_unused]] auto success = Invoke("itemToLabel", std::addressof(str), args, kNumArgs);
				assert(success);
			...
			{
				[[maybe_unused]] auto success = Invoke("open");
				assert(success);
			}
			...
			{
				[[maybe_unused]] auto success = Invoke("close");
				assert(success);
			}
			...
			{
				[[maybe_unused]] auto success = Invoke("invalidateData");
				assert(success);
			}
			...
			void DropdownMenu::SetSize(double a_width, double a_height)
			{
				...
				[[maybe_unused]] auto success = Invoke("setSize", nullptr, args, kNumArgs);
				assert(success);
			}
			...
			{
				[[maybe_unused]] auto success = Invoke("removeMovieClip");
				assert(success);
			}
