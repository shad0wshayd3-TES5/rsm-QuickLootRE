...
namespace CLIK
{
	namespace GFx
	{
		namespace Core
		{
			...
			{
				[[maybe_unused]] auto success = Invoke("invalidate");
				assert(success);
			}

			void UIComponent::ValidateNow()
			{
				[[maybe_unused]] auto success = Invoke("validateNow");
				assert(success);
			}
			...
			void UIComponent::DispatchEventToGame(Object& a_event)
			{
				...
				[[maybe_unused]] auto success = Invoke("dispatchEventToGame", nullptr, args, kNumArgs);
				assert(success);
			}
		}
	}
